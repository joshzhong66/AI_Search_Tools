(() => {
  ...
  function renderDouyinCollectedComments() {
    ...
    return `<section class="douyin-detail-comments"><header><strong>评论 ${number(items.length)}</strong><span>本次采集结果</span></header>${items.length ? `<div class="douyin-comment-list">${items.map((comment) => { const user = comment.user || comment.author || {}; return `<article class="douyin-comment-item"><div class="identity-cell compact-identity">${image(user.avatarThumb || user.avatar || user.avatar_url, user.nickname || user.name, "comment-avatar")}<div><strong>${escapeHtml(user.nickname || user.name || "未知用户")}</strong><small>${escapeHtml(comment.region || comment.ipLocation || "")}</small></div></div><p>${escapeHtml(comment.text || comment.content || "-")}</p><footer><span>点赞 ${number(comment.likeCount || comment.like_count)}</span><span>回复 ${number(comment.replyCount || comment.reply_count)}</span><span>${escapeHtml(formatDate(comment.createDate || comment.createTime))}</span></footer></article>`; }).join("")}</div>` : `<div class="empty-comments"><strong>没有公开评论</strong><span>任务已完成，但未返回评论数据。</span></div>`}</section>`;
  }
