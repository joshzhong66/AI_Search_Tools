(() => {
  ...
  const state = {
    ...
    xhsPrefill: {}, weiboPrefill: {}, kuaishouPrefill: {}, douyinPrefillUrls: [], activeNote: null, activeDouyinVideo: null, douyinDetailError: "", douyinDetailBusy: false, douyinCommentTaskId: "", commentTaskId: "", commentTaskIds: [],
  ...
  function renderKuaishouData(taskMeta, detail, payload) {
    ...
    if (["kuaishou_search_videos", "kuaishou_get_video_detail", "kuaishou_list_user_videos"].includes(operation)) {
      const columns = [{ label: "视频", render: (item) => `<div class="comment-content"><strong>${escapeHtml(kuaishouValue(item, ["title", "caption", "description", "text"], "未命名视频"))}</strong><small>${escapeHtml(kuaishouVideoId(item) || "-")}</small></div>` }, { label: "作者", render: (item) => escapeHtml(kuaishouUserName(item)) }, { label: "点赞", render: (item) => number(kuaishouCount(item, ["like_count", "liked_count", "real_like_count"])), numeric: true }, { label: "评论", render: (item) => number(kuaishouCount(item, ["comment_count", "commentCount"])), numeric: true }, { label: "发布时间", render: (item) => escapeHtml(formatDate(kuaishouValue(item, ["publish_time", "created_at", "create_time", "timestamp"]))) }];
      return `<div class="metrics">${metric("视频", number(items.length), "当前结果")}${metric("点赞", number(items.reduce((sum, item) => sum + kuaishouCount(item, ["like_count", "liked_count", "real_like_count"]), 0)), "合计")}${metric("评论", number(items.reduce((sum, item) => sum + kuaishouCount(item, ["comment_count", "commentCount"]), 0)), "可继续采集")}${metric("任务状态", detail.status === "settled" ? "已结算" : formatValue(detail.status), "上游状态")}</div>${table(columns, items, taskId, (item, index) => `<button class="button primary compact" type="button" data-kuaishou-comments data-video-id="${escapeAttr(kuaishouVideoId(item))}" data-video-url="${escapeAttr(kuaishouVideoUrl(item))}">采集评论</button>${kuaishouVideoUrl(item) ? `<a class="button secondary compact" href="${escapeAttr(kuaishouVideoUrl(item))}" target="_blank" rel="noopener">原文</a>` : ""}<button class="button secondary compact" type="button" data-detail-index="${index}" data-task-id="${escapeAttr(taskId)}">详情</button>`)}`;
    }
